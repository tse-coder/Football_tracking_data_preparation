...
class FrameExtractor:
    ...
    def extract_frames(
        ...
        print(f"Targeting {self.target_fps} FPS (Interval: {self.time_interval_sec:.3f}s), max {frame_limit} frames.")
        ...
        try:
            while self.vid_capture.is_opened():
                ...
                current_time_sec = self.vid_capture.video.get(cv2.CAP_PROP_POS_MSEC) / 1000.0
                ...
                is_pitch, green_ratio = FrameFilters.is_pitch_frame(resized, MIN_GREEN_RATIO)
                is_exposed, brightness_score = FrameFilters.is_exposed_properly(resized, MIN_BRIGHTNESS, MAX_BRIGHTNESS)
                
                curr_hist = FrameFilters.compute_histogram(resized) if (filter_transitions or filter_replays) else None
                is_transition = False
                
                if prev_hist is not None and curr_hist is not None:
                     is_transition = FrameFilters.is_scene_transition(prev_hist, curr_hist, SCENE_CHANGE_THRESHOLD)

                stats["sum_blur"] += blur_score
                stats["sum_green"] += green_ratio

                # Apply filters hierarchically
                if filter_blurry and is_blurry:
                ...
                if filter_replays and FrameFilters.detect_replay_heuristic(is_transition, green_ratio, MIN_GREEN_RATIO):
                ...
                last_filename = f"frame_{saved_count:06d}_sec_{current_time_sec:.2f}.jpg"
